...
export default class Edge extends React.Component {
  render() {
    ...
    const x1 = to.x - alterX1 * Math.cos(theta1) * radius1 || 0;
    const y1 = to.y - alterY1 * Math.sin(theta1) * radius1 || 0;
    ...
    const x2 = from.x - alterX2 * Math.cos(theta2) * radius2 || 0;
    const y2 = from.y - alterY2 * Math.sin(theta2) * radius2 || 0;

    const fill = this.props.path ? "#99ccff" : "#ef4f6c";
    ...
    return (
      ...
      >
        <defs>
          <marker
            id={`arrow-${this.props.from.index}-${this.props.to.index}`}
            markerWidth={20}
            markerHeight={20}
            refX="0"
            refY="3"
        ...
        <line
          ...
          markerEnd={`url(#arrow-${this.props.from.index}-${
            this.props.to.index
          }`}
        />
      </svg>
